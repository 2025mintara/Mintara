...
export default function AINFTBuilderPage() {
    ...
    <div className="min-h-screen pt-32 px-6 py-16">
      <div className="max-w-[1200px] mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-semibold text-mintara-text-primary mb-4">Create AI-Generated NFTs</h1>
          <p className="text-3xl md:text-4xl text-mintara-accent mt-3">Mint Instantly on Base.</p>
          <div className="h-4" />
          <p className="text-xl text-mintara-text-secondary mb-6">Pay only 1 USDC per NFT mint.</p>
        ...
        <Card className="p-8 md:p-12 bg-mintara-surface/50 border-mintara-border backdrop-blur-sm mb-12">
          <div className="space-y-6">
            <div className="space-y-3">
              <label className="text-mintara-text-primary font-medium">Describe Your NFT</label>
              <Textarea
            ...
            <div className="space-y-2">
              <label className="text-sm text-mintara-text-secondary">Recent Prompts:</label>
            ...
            <Button onClick={handleGenerate} disabled={!prompt || isGenerating} size="lg" className="w-full">
              {isGenerating ? (
                <>
          ...
          {isGenerating && (
            <div className="mt-12 flex flex-col items-center justify-center py-16">
              <div className="w-24 h-24 rounded-full bg-gradient-to-br from-mintara-primary via-mintara-accent to-mintara-soft-lime-glow animate-spin mb-6" />
              <p className="text-lg text-mintara-text-primary font-medium">Generating NFT with AI...</p>
            </div>
          )}

          {previews && !isGenerating && (
            <div className="mt-12 space-y-6">
              <h3 className="text-2xl font-semibold text-mintara-text-primary">Your Generated NFTs</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[1, 2, 3].map((index) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-mintara-text-secondary text-center">NFT Preview #{index}</p>
                  </Card>
                ))}
              </div>

              <Card className="p-6 bg-mintara-background border-mintara-border">
                <h4 className="text-lg font-semibold text-mintara-text-primary mb-4">NFT Metadata</h4>
                <div className="grid gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) => setNftMetadata({ ...nftMetadata, name: event.target.value })}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) => setNftMetadata({ ...nftMetadata, description: event.target.value })}
                      className="bg-mintara-background border-mintara-border text-mintara-text-primary focus:border-mintara-accent"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) => setNftMetadata({ ...nftMetadata, collection: event.target.value })}
                      className="bg-mintara-background border-mintara-border text-mintara-text-primary focus:border-mintara-accent"
                    />
                  </div>
                </div>
              ...
              <Button onClick={handleMint} size="lg" variant="outline" className="w-full" disabled={!isConnected}>
                Mint Now
              </Button>
              {!isConnected && (
        ...
        <div>
          <h2 className="text-2xl font-semibold text-mintara-text-primary mb-8 text-center">Powered by Advanced AI</h2>
            ...
            {features.map((feature) => (
              ...
              >
                <div className="flex flex-col items-center text-center gap-4">
                  ...
                  <h3 className="text-lg font-semibold text-mintara-text-primary">{feature.title}</h3>
                  <p className="text-mintara-text-secondary text-sm">{feature.description}</p>
                </div>
              </Card>
            ))}
          </div>
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent className="bg-mintara-surface border-mintara-border max-w-md">
          <DialogHeader>
            ...
            <DialogTitle className="text-mintara-text-primary text-center text-2xl">?? NFT Minted Successfully!</DialogTitle>
