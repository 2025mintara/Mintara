...
import { Coins, ExternalLink, MessageCircle, Share2, X } from "lucide-react";
...
} satisfies Record<string, { supply: string; decimals: string; liquidity: string; vesting: string }>;

export default function DashboardPage() {
  const [chatOpen, setChatOpen] = useState(false);
  const [tokenType, setTokenType] = useState<keyof typeof tokenomicsRecommendations>("meme");
  ...
  return (
    <div className="min-h-screen pt-32 px-6 py-16">
      <div className="max-w-[1200px] mx-auto">
        <h1 className="text-4xl md:text-5xl font-semibold text-mintara-text-primary mb-2">Dashboard</h1>
        <p className="text-xl text-mintara-text-secondary mb-12">My Creations & AI Assistant</p>

        <Tabs defaultValue="creations" className="space-y-8">
          <TabsList className="bg-mintara-surface/50 border border-mintara-border p-1">
            <TabsTrigger value="creations" className="data-[state=active]:bg-mintara-primary data-[state=active]:text-white">
              My Creations
            </TabsTrigger>
            <TabsTrigger value="tokenomics" className="data-[state=active]:bg-mintara-primary data-[state=active]:text-white">
              AI Tokenomics Designer
            </TabsTrigger>
            <TabsTrigger value="credits" className="data-[state=active]:bg-mintara-primary data-[state=active]:text-white">
              Credits
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="creations" className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-semibold text-mintara-text-primary">{item.name}</h3>
                        <p className="text-sm text-mintara-text-secondary">{item.symbol}</p>
                      </div>
                    </div>
                  ...
                  <p className="text-sm text-mintara-text-secondary mb-4 font-mono">{item.address}</p>
            ...
            {myTokens.length === 0 && (
              <Card className="p-12 bg-mintara-surface/50 border-mintara-border text-center">
                <p className="text-mintara-text-secondary">No creations yet. Start by creating a token or NFT!</p>
              </Card>
            )}
          </TabsContent>
          ...
          <TabsContent value="tokenomics" className="space-y-6">
            <Card className="p-8 bg-mintara-surface/50 border-mintara-border backdrop-blur-sm">
              <h2 className="text-2xl font-semibold text-mintara-text-primary mb-6">AI Tokenomics Designer</h2>
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-mintara-text-primary font-medium">Select Token Type</label>
                  <Select value={tokenType} onValueChange={(value: keyof typeof tokenomicsRecommendations) => setTokenType(value)}>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-mintara-border">
                        <th className="text-left py-3 px-4 text-mintara-text-primary font-medium">Parameter</th>
                        <th className="text-left py-3 px-4 text-mintara-text-primary font-medium">Recommendation</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr className="border-b border-mintara-border/50">
                        <td className="py-3 px-4 text-mintara-text-secondary">Supply</td>
                        <td className="py-3 px-4 text-mintara-text-primary font-mono">
                          {tokenomicsRecommendations[tokenType].supply}
                        </td>
                      </tr>
                      <tr className="border-b border-mintara-border/50">
                        <td className="py-3 px-4 text-mintara-text-secondary">Decimals</td>
                        <td className="py-3 px-4 text-mintara-text-primary font-mono">
                          {tokenomicsRecommendations[tokenType].decimals}
                        </td>
                      </tr>
                      <tr className="border-b border-mintara-border/50">
                        <td className="py-3 px-4 text-mintara-text-secondary">Liquidity</td>
                        <td className="py-3 px-4 text-mintara-text-primary">
                          {tokenomicsRecommendations[tokenType].liquidity}
                        </td>
                      </tr>
                      <tr>
                        <td className="py-3 px-4 text-mintara-text-secondary">Vesting</td>
                        <td className="py-3 px-4 text-mintara-text-primary">
          ...
          <TabsContent value="credits" className="space-y-6">
            <Card className="p-12 bg-mintara-surface/50 border-mintara-border backdrop-blur-sm text-center">
              ...
              <h2 className="text-3xl font-semibold text-mintara-text-primary mb-2">7 Credits</h2>
              <p className="text-mintara-text-secondary mb-8">10 credits = 1 free mint</p>
              <div className="max-w-md mx-auto">
                ...
                <p className="text-sm text-mintara-text-secondary">3 more credits until your next free mint!</p>
              </div>
            </Card>
          </TabsContent>
        </Tabs>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        {chatOpen ? (
          <Card className="w-80 h-96 bg-mintara-surface border-mintara-border backdrop-blur-xl shadow-2xl flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-mintara-border">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-mintara-text-primary">Ask Mintara AI</span>
              </div>
              <button
            ...
            <div className="flex-1 p-4 overflow-y-auto">
              <div className="bg-mintara-background rounded-lg p-3 mb-3">
                <p className="text-sm text-mintara-text-primary">Hi! I'm Mintara AI. How can I help you today?</p>
              </div>
            </div>
          ...
          <button
            ...
            aria-label="Open Mintara AI chat"
          >
